...
const leadTimes  = require('../getVendorsByReference/filterLeadTimes.js');
...
function processAction(message, config) {
  ...
  // submits the HTTP request to the VMS to retrieve all vendors for given references
  ...
  Promise.all([requestOptions(config), getVendorReferences(message)]).
    ...
    then(handleResponse).
    then(leadTimes.filterLeadTimes).
    then(emitData).
    catch(handleError).
    then(emitEnd);
}

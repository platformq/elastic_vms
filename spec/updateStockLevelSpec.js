...
function processAction(message, config) {
  ...
  function acknowledgeSubOrder(authenticationHeaders) {
    let { id } = message.body.currentMessage;
    let { quantity } = message.body.cassellie.getStockLevel;

    let options = Object.assign({}, authenticationHeaders, {
      url: `${authenticationHeaders.url}sub-orders/${id}`,
      json: true,
      body: { data: { type: 'variants', id: id, attributes: { acknowledge: true } } }
    });
    console.log(options);
  ...
  function emitData(response) {
    ...
    message.body.vms.acknowledgeSubOrder = { vmsSubOrderAcknowledgement: body };
